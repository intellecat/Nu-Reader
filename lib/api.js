...
exports.rss = function(req,res) {
	var RSS = require('rss');
	db.collection('jobs').findById(req.params.job_id,function(err,job){
		var feed = new RSS({
			title: job.data.title
		});
		db.collection('posts').find({job:job.name}).sort({pubDate:-1}).limit(20).toArray(function(err,posts){
